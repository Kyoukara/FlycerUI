...
return function(Config)
	...
	if not RunService:IsStudio() and Window.Folder and writefile then
		if not isfolder("FlycerUI/" .. Window.Folder) then
			makefolder("FlycerUI/" .. Window.Folder)
		end
		if not isfolder("FlycerUI/" .. Window.Folder .. "/assets") then
			makefolder("FlycerUI/" .. Window.Folder .. "/assets")
		end
	...
	if typeof(Window.Background) == "string" and BGVideo then
		...
		if string.find(BGVideo, "http") then
			...
			if not isfile(videoPath) then
				...
				if not success then
					warn("[ FlycerUI.Window.Background ] Failed to download video: " .. tostring(result))
				end
			end
			...
			if not success then
				warn("[ FlycerUI.Window.Background ] Failed to load custom asset: " .. tostring(customAsset))
			end
	...
	Window.UIElements.Main = New("Frame", {
		...
		Config.FlycerUI.UIScaleObj,
		Window.AcrylicPaint and Window.AcrylicPaint.Frame or nil,
		Blur,
		Creator.NewRoundFrame(Window.UICorner, "Squircle", {
			ImageTransparency = 1, --  Window.Transparent and Config.FlycerUI.TransparencyValue or 0,
			Size = UDim2.new(1, 0, 1, 0),
			AnchorPoint = Vector2.new(0.5, 0.5),
			Position = UDim2.new(0.5, 0, 0.5, 0),
			Name = "Background",
	...
	Creator.AddSignal(Window.UIElements.Main.Main.Topbar.Left:GetPropertyChangedSignal("AbsoluteSize"), function()
		...
		local RightWidth = Window.UIElements.Main.Main.Topbar.Right.UIListLayout.AbsoluteContentSize.X
			/ Config.FlycerUI.UIScale

		LeftWidth = Window.UIElements.Main.Main.Topbar.Left.AbsoluteSize.X / Config.FlycerUI.UIScale
		...
		Window.UIElements.Main.Main.Topbar.Center.Position =
			UDim2.new(0, LeftWidth + (Window.UIPadding / Config.FlycerUI.UIScale), 0.5, 0)
		Window.UIElements.Main.Main.Topbar.Center.Size = UDim2.new(
			1,
			-LeftWidth
				- (Window.UIPadding / Config.FlycerUI.UIScale)
	...
	if Window.Topbar.ButtonsType ~= "Default" then
		Creator.AddSignal(Window.UIElements.Main.Main.Topbar.Right:GetPropertyChangedSignal("AbsoluteSize"), function()
			Window.UIElements.Main.Main.Topbar.Left.Position = UDim2.new(
				0,
				(Window.UIElements.Main.Main.Topbar.Right.AbsoluteSize.X / Config.FlycerUI.UIScale) + Window.UIPadding - 4,
				0,
				0
			)
		end)
	...
	if not IsVideoBG and Window.Background and typeof(Window.Background) == "table" then
		...
		Window.UIElements.BackgroundGradient = Creator.NewRoundFrame(Window.UICorner, "Squircle", {
			...
			ImageTransparency = Window.Transparent and Config.FlycerUI.TransparencyValue or 0,
		}, {
			BackgroundGradient,
		})
	end
	...
	function Window:SetBackgroundTransparency(v)
		...
		Config.FlycerUI.TransparencyValue = rounded
	...
	local function SetSize(isAnimation)
		Tween(Window.UIElements.Main, 0.45, {
			Size = not Window.IsFullscreen and CurrentSize or UDim2.new(
				0,
				(Config.FlycerUI.ScreenGui.AbsoluteSize.X - 20) / Config.FlycerUI.UIScale,
				0,
				(Config.FlycerUI.ScreenGui.AbsoluteSize.Y - 20 - 52) / Config.FlycerUI.UIScale
			),
		}, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
	...
	Creator.AddSignal(Config.FlycerUI.ScreenGui:GetPropertyChangedSignal("AbsoluteSize"), function()
	...
	if Config.FlycerUI.UseAcrylic then
	...
	function Window:Open()
		...
		task.spawn(function()
			...
			--[[Config.FlycerUI.UIScaleObj.Scale -= 1 - 0.85
			Tween(
				Config.FlycerUI.UIScaleObj,
				0.33,
				{ Scale = Config.FlycerUI.UIScale },
			...
			Config.FlycerUI:ToggleAcrylic(true)
			--end)
		end)
	end
	function Window:Close()
		...
		Config.FlycerUI:ToggleAcrylic(false)
		...
		--[[Tween(
			Config.FlycerUI.UIScaleObj,
			0.28,
			{ Scale = Config.FlycerUI.UIScale - (1 - 0.85) },
			Enum.EasingStyle.Quint,
			Enum.EasingDirection.Out
		):Play()]]
		if BGImage then
		...
		function Close:Destroy()
			task.spawn(function()
				...
				Config.FlycerUI.ScreenGui:Destroy()
				Config.FlycerUI.NotificationGui:Destroy()
				Config.FlycerUI.DropdownGui:Destroy()
				Config.FlycerUI.TooltipGui:Destroy()
	...
	function Window:ToggleTransparency(Value)
		...
		Config.FlycerUI.Transparent = Value

		Window.UIElements.Main.Background.ImageTransparency = Value and Config.FlycerUI.TransparencyValue or 0
		-- Window.UIElements.Main.Background.ImageLabel.ImageTransparency = Value and Config.FlycerUI.TransparencyValue or 0
	...
	function Window:GetUIScale(v)
		return Config.FlycerUI.UIScale
	end

	function Window:SetUIScale(v)
		Config.FlycerUI.UIScale = v
		Tween(Config.FlycerUI.UIScaleObj, 0.2, { Scale = v }, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
		return Window
	end
	...
	local TabModule = TabModuleMain.Init(Window, Config.FlycerUI, Config.FlycerUI.TooltipGui)
	...
	function Window:Tab(TabConfig)
		...
		return TabModule.New(TabConfig, Config.FlycerUI.UIScale)
	end
	...
	function Window:Dialog(DialogConfig)
		...
		local Dialog = DialogModule.Create(false, "Dialog", Window, Config.FlycerUI, Window.UIElements.Main.Main)
		...
		local function CheckButtonsOverflow()
			...
			local totalWidth = ButtonsLayout.AbsoluteContentSize.X / Config.FlycerUI.UIScale
			local parentWidth = ButtonsContent.AbsoluteSize.X / Config.FlycerUI.UIScale
			...
			if totalWidth > parentWidth then
				...
				if availableSpace > 0 then
					...
					for _, button in ipairs(Buttons) do
						local buttonWidth = button.AbsoluteSize.X / Config.FlycerUI.UIScale
	...
	local CurResizeInput = Config.FlycerUI.GenerateGUID()
		...
		if Window.CanResize then
			...
			Creator.AddSignal(input.Changed, function()
				if input.UserInputState == Enum.UserInputState.End then
					if Config.FlycerUI.CurrentInput and Config.FlycerUI.CurrentInput ~= CurResizeInput then
						return
					end

					Config.FlycerUI.CurrentInput = nil
	...
	Creator.AddSignal(ResizeHandle.InputBegan, function(input)
		...
		then
			...
			Config.FlycerUI.CurrentInput = CurResizeInput
