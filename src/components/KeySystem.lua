...
local function GetFlycerIdentifier(Config)
	if type(Config) ~= "table"
		or type(Config.KeySystem) ~= "table" then
	...
	if not serviceInstance then
		return nil,
			"Invalid",
			serviceError or "Flycer service is not available."
	end
	...
	local ok,
		identifier,
		identifierType,
		identifierError = pcall(function()
	...
	if not identifier or tostring(identifier) == "" then
		return nil,
			identifierType or "Invalid",
			identifierError or "Unable to determine Flycer identifier."
	end
...
CreateFlycerService = function(Config)
	local flycerConfig =
		Config.KeySystem
		and Config.KeySystem.Flycer

	if type(flycerConfig) ~= "table" then
		return nil, "Flycer configuration is missing."
	end

	if not flycerConfig.Endpoint
		or tostring(flycerConfig.Endpoint) == "" then

		return nil, "Flycer API Endpoint is not configured."
	end

	local serviceData =
		Config.FlycerUI.Services.flycer

	if not serviceData
		or type(serviceData.New) ~= "function" then
	...
	return serviceData.New(
		...
		flycerConfig.LockType
			or Config.KeySystem.LockType
			or "Device",
...
local function OpenFlycerServiceDialog(
	...
	if KeyDialog
		and KeyDialog.UIElements.MainContainer then
	...
	local CopyButton = CreateButton(
		...
		function()
			if Identifier
				and CopyToClipboard(Identifier) then

				Config.FlycerUI:Notify({
					Title = "Flycer",
					Content =
						tostring(IdentifierType)
	...
	local Discord =
		Config.KeySystem.Discord
		or Config.KeySystem.DiscordURL
...
function KeySystem.new(Config, Filename, func, keyValidator)
	...
	local ThumbnailSize =
		(Config.KeySystem.Thumbnail
			and Config.KeySystem.Thumbnail.Width)
		or 200
	...
	if Config.KeySystem.Thumbnail
		and Config.KeySystem.Thumbnail.Image then
	...
	KeyDialog.UIElements.Main.Size =
		UDim2.new(0, UISize, 0, 0)
	...
	if Config.KeySystem.Note
		and Config.KeySystem.Note ~= "" then
	...
	if Config.KeySystem.Thumbnail
		...
		if Config.KeySystem.Thumbnail.Title then
			ThumbnailTitle = New("TextLabel", {
				Text =
					Config.KeySystem.Thumbnail.Title,
		...
		ThumbnailFrame = New("ImageLabel", {
			Image =
				Config.KeySystem.Thumbnail.Image,
	...
	if Config.KeySystem.URL
		and not Config.KeySystem.KeyValidator then
	...
	if Config.KeySystem.API
		or Config.KeySystem.KeyValidator
		or type(Config.KeySystem.Flycer) == "table" then
		...
		local function AddFlycerService()
			...
			Creator.AddSignal(
				APIFrame.MouseButton1Click,
				function()

					local Identifier,
						IdentifierType,
						IdentifierError =
						GetFlycerIdentifier(Config)
		...
		if Config.KeySystem.KeyValidator
			or type(Config.KeySystem.Flycer) == "table" then
		...
		for _, i in next, (Config.KeySystem.API or {}) do
			...
			if i.Type ~= "flycer" then
				...
				local serviceDef =
					Config.FlycerUI.Services[i.Type]
						...
						0.3,
						{
							Size =
								UDim2.new(
									...
									Width,
									0,
									DropdownFrame.AbsoluteSize.Y + 1
								)
						},
		...
		CreateButton(
			...
			function()
				...
				if Config.KeySystem.KeyValidator then
					...
					local isValid,
						validationMessage =
						Config.KeySystem.KeyValidator(key)
				...
				elseif not Config.KeySystem.API then
					...
					local isKey =
						type(Config.KeySystem.Key) == "table"
						and table.find(
							Config.KeySystem.Key,
							key
						)
