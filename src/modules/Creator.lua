...
function Creator.Init(FlycerUITable)
	FlycerUI = FlycerUITable

	Creator.ThemeFallbacks = require("../themes/Fallbacks")(Creator)

	Creator.UIScale = FlycerUITable.UIScale
...
function Creator.SafeCallback(Function, ...)
	...
	if not Success then
		if FlycerUI and FlycerUI.Window and FlycerUI.Window.Debug then
			local _, i = Event:find(":%d+: ")

			warn("[ FlycerUI: DEBUG Mode ] " .. Event)

			return FlycerUI:Notify({
...
function Creator.Gradient(stops, props)
	if FlycerUI and FlycerUI.Gradient then
		return FlycerUI:Gradient(stops, props)
	end
...
function Creator.Drag(mainFrame, dragFrames, ondrag)
	...
	for _, dragFrame in pairs(dragFrames) do
		dragFrame.InputBegan:Connect(function(input)
			...
			then
				if FlycerUI and FlycerUI.CurrentInput and FlycerUI.CurrentInput ~= CurInput then
					return
				end
	...
	UserInputService.InputEnded:Connect(function(input)
		if not dragging or FlycerUI.CurrentInput ~= CurInput then
			return
		end
...
function Creator.Image(Img, Name, Corner, Folder, Type, IsThemeTag, Themed, ThemeTagName)
	...
	elseif string.find(Img, "http") and not string.find(Img, "roblox.com") then
		local FileName = "FlycerUI/" .. Folder .. "/assets/." .. Type .. "-" .. Name .. ".png"
