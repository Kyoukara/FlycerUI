...
-- TRANSPARENCY & EXCLUDED (diperlukan oleh Reveal System)
local TRANSPARENCY_PROPS_GLOBAL = {
...
function FlycerLib:CreateWindow(windowCfg)
    ...
    local function cacheRevealTarget(obj)
        ...
        if EXCLUDED_CLASSES_GLOBAL[obj.ClassName] then return end
        local propList = TRANSPARENCY_PROPS_GLOBAL[obj.ClassName]
    ...
    local function revealUI(callback)
        ...
        local function onComplete()
            ...
            if activeTweens <= 0 then
                callbackFired = true
                for obj2, props2 in pairs(revealTargets) do
                    if obj2 and obj2.Parent then
                        for p, v in pairs(props2) do obj2[p] = v end
                    end
                end
                if callback then task.spawn(callback) end
            end
